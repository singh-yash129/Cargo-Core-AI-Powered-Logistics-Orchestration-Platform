    ...
    <BaseModal :is-open="isOpen" @close="$emit('close')">
        ...
        <div class="grid grid-cols-1 gap-3">
            <button @click="selectWarehouse('all')"
                ...
                :class="activeId === 'all' ? 'bg-primary/10 border-primary' : 'bg-white/5 border-white/5 hover:bg-white/10 hover:border-white/20'">
                <div class="w-10 h-10 rounded-lg flex items-center justify-center"
                    :class="activeId === 'all' ? 'bg-primary/20 text-primary' : 'bg-gray-700 text-gray-400'">
                    <span class="material-symbols-outlined">public</span>
                </div>
                <div>
                    <div class="font-bold text-white">All Warehouses</div>
                    <div class="text-xs text-gray-400 mt-0.5">Global View</div>
                </div>
            ...
            <div v-for="hub in hubs" :key="hub.id" @click="selectWarehouse(hub.id)"
                ...
                :class="activeId === hub.id ? 'bg-primary/10 border-primary' : 'bg-white/5 border-white/5 hover:bg-white/10 hover:border-white/20'">
