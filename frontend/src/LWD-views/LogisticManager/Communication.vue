<template>
    <div class="h-[calc(100vh-8rem)] flex gap-6">
        ...
        <!-- Active Chat Area -->
        <div v-if="activeChat" class="flex-1 glass-panel rounded-xl flex flex-col overflow-hidden">
            <div
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                             {{ activeChat.name }}
                             <span v-if="activeChat.muted" class="material-symbols-outlined text-gray-400 text-[16px]" title="Chat Muted">notifications_off</span>
                        </div>
                        <div class="text-xs flex items-center gap-2">
                            <span class="flex items-center gap-1" :class="activeChat.status === 'Online' ? 'text-green-500' : 'text-gray-400'">
                                <span class="w-1.5 h-1.5 rounded-full" :class="activeChat.status === 'Online' ? 'bg-green-500' : 'bg-gray-400'"></span> 
                                {{ activeChat.status }}
                            </span>
                            <span v-if="activeChat.phone" class="text-gray-400 border-l border-gray-300 dark:border-gray-600 pl-2 font-mono">{{ activeChat.phone }}</span>
                        </div>
                    </div>
                </div>
                <div class="flex gap-2 relative">
                    ...
                    <!-- Dropdown Menu -->
                    <div v-if="isMenuOpen" class="absolute right-0 top-12 w-48 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-white/10 overflow-hidden z-50 animate-fade-in-up origin-top-right">
                        <button class="w-full text-left px-4 py-3 text-sm hover:bg-gray-50 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300 flex items-center gap-3 transition-colors" @click="handleViewProfile">
                            <span class="material-symbols-outlined text-[18px] text-gray-400">person</span> View Profile
                        </button>
                        <button class="w-full text-left px-4 py-3 text-sm hover:bg-gray-50 dark:hover:bg-white/5 text-gray-700 dark:text-gray-300 flex items-center gap-3 transition-colors" @click="handleMuteChat">
                            <span class="material-symbols-outlined text-[18px] text-gray-400">{{ activeChat.muted ? 'notifications' : 'notifications_off' }}</span> {{ activeChat.muted ? 'Unmute Chat' : 'Mute Chat' }}
                        </button>
                        <button class="w-full text-left px-4 py-3 text-sm hover:bg-red-50 dark:hover:bg-red-900/10 text-red-600 dark:text-red-400 flex items-center gap-3 transition-colors" @click="handleDeleteChat">
            ...
            <div ref="chatHistoryContainer" class="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/30 dark:bg-black/10 scroll-smooth">
                <div v-for="msg in activeChat.messages" :key="msg.id" class="flex gap-3" :class="{'flex-row-reverse': msg.sender === 'me'}">
                    <div
                        class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold shadow-sm"
                    ...
                    <div class="max-w-[70%]">
                        <div
                            class="p-3 rounded-xl text-sm shadow-sm"
                            :class="msg.sender === 'me' 
                        ...
                        <div class="text-[10px] text-gray-400 mt-1" :class="{'text-right': msg.sender === 'me'}">{{ msg.time }}</div>
                    </div>
                </div>
            </div>
        ...
        <!-- Profile Modal -->
        <div v-if="isProfileModalOpen && activeChat" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
             <div class="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-gray-100 dark:border-white/10 flex flex-col md:flex-row h-[500px]">
                 <!-- Left: Profile Details -->
                 <div class="w-full md:w-1/2 p-8 bg-gray-50 dark:bg-white/5 flex flex-col items-center justify-center text-center border-r border-gray-100 dark:border-white/10">
                     <div class="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-3xl font-bold text-white mb-4 shadow-lg shadow-purple-500/30">
                     ...
                     <div class="flex gap-2 mb-6">
                         <span class="px-3 py-1 rounded-full text-xs font-bold border border-gray-200 dark:border-white/10"
                         ...
                         <span v-if="activeChat.muted" class="px-3 py-1 rounded-full text-xs font-bold bg-yellow-50 text-yellow-600 border border-yellow-200 dark:bg-yellow-500/10 dark:text-yellow-400 dark:border-yellow-500/20 flex items-center gap-1">
                 ...
                 <div class="w-full md:w-1/2 p-8 flex flex-col">
                     <div class="flex justify-between items-center mb-6">
                         ...
                         <button @click="isProfileModalOpen = false" class="text-gray-400 hover:text-gray-600"><span class="material-symbols-outlined">close</span></button>
                     </div>
                     ...
                     <div class="space-y-4 mb-auto">
                         ...
                         <div class="bg-purple-50 dark:bg-purple-900/10 p-4 rounded-xl border border-purple-100 dark:border-purple-500/20">
                             <div class="flex justify-between items-center mb-1">
                                 <span class="text-xs font-bold text-purple-600 dark:text-purple-400 uppercase">On-Time Deliveries</span>
                     ...
                     <div class="grid grid-cols-2 gap-3 mt-6">
                         <button @click="handleMuteChat" class="px-4 py-3 bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-700 dark:text-gray-300 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2">
                             <span class="material-symbols-outlined text-[18px]">{{ activeChat.muted ? 'notifications' : 'notifications_off' }}</span> {{ activeChat.muted ? 'Unmute' : 'Mute' }}
                         </button>
                         <button @click="isProfileModalOpen = false" class="px-4 py-3 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2">
        ...
        <!-- Delete Modal -->
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
             <div class="bg-white dark:bg-gray-900 w-full max-w-sm rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-white/10 text-center">
                 ...
                 <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Are you sure you want to delete the chat with <span class="font-bold text-gray-900 dark:text-white">{{ activeChat?.name }}</span>? This action cannot be undone.</p>
                 
                 <div class="flex gap-3 justify-center">
                     <button @click="isDeleteModalOpen = false" class="px-5 py-2.5 rounded-xl font-bold text-sm text-gray-600 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 dark:text-gray-300 transition-colors">Cancel</button>
                     <button @click="confirmDeleteChat" class="px-5 py-2.5 rounded-xl font-bold text-sm text-white bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30 transition-colors">Delete Chat</button>
                 </div>
             </div>
        </div>
    </div>
...
const activeChat = computed(() => {
    return filteredChats.value.find(c => c.id === activeChatId.value) || filteredChats.value[0]
})
