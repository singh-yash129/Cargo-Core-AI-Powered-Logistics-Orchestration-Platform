<template>
    <div class="h-full flex flex-col space-y-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
            ...
            <div class="flex flex-wrap gap-2 items-center">
                 ...
                 <div class="relative group mr-2">
                    <input v-model="aiQuery" @keyup.enter="handleAiQuery" type="text" placeholder="Ask AI: 'Show me top spending vendors...'" class="pl-10 pr-4 py-2 bg-white dark:bg-white/5 border border-purple-200 dark:border-purple-500/30 rounded-full text-sm w-64 focus:w-80 transition-all outline-none focus:ring-2 focus:ring-purple-500/50 shadow-sm">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-purple-500">auto_awesome</span>
                </div>

                <select v-model="timeRange" class="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-sm px-3 py-2 outline-none focus:ring-2 focus:ring-primary/50 text-gray-700 dark:text-gray-200 cursor-pointer">
                ...
                <button @click="downloadCurrentView" class="bg-white hover:bg-gray-50 dark:bg-white/10 dark:hover:bg-white/20 text-gray-700 dark:text-white font-bold px-4 py-2 rounded-lg text-sm flex items-center gap-2 transition-all border border-gray-200 dark:border-white/10">
        ...
        <!-- Navigation Tabs -->
        <div class="flex gap-1 overflow-x-auto pb-2 shrink-0 border-b border-gray-200 dark:border-white/10 no-scrollbar">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-10">
            ...
            <div v-if="activeTab === 'control_tower'" class="space-y-6 animate-fade-in">
                ...
                <div v-if="crisisAlerts.length > 0" class="flex gap-4 overflow-x-auto pb-2">
                    <div v-for="(alert, i) in crisisAlerts" :key="i" class="flex-shrink-0 w-80 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 p-4 rounded-xl flex items-start gap-3 shadow-sm hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    <!-- Daily Orders Gauge -->
                    <div class="lg:col-span-1 p-6 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm text-center relative overflow-hidden group hover:scale-[1.02] transition-transform">
                        <h3 class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-4">Daily Completion</h3>
                    ...
                    <!-- System Health + Dues -->
                     <div class="lg:col-span-1 space-y-4">
                        <div class="p-5 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
                            <h3 class="text-xs font-bold text-gray-500 uppercase mb-2">System Health</h3>
                            <div class="flex items-end gap-2">
                                <span class="text-4xl font-black text-green-500">{{ store.dashboardStats.deliverySuccess }}%</span>
                                <span class="text-xs text-green-600 mb-1 font-bold bg-green-100 px-2 py-0.5 rounded-full">+0.4%</span>
                            </div>
                            <p class="text-xs text-gray-400 mt-2">Delivery Success Rate</p>
                        </div>
                         <div class="p-5 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-500/20 rounded-2xl shadow-sm relative overflow-hidden group hover:scale-[1.02] transition-transform">
                            <div class="absolute -right-4 -top-4 bg-red-500 w-16 h-16 rounded-full opacity-20 blur-xl group-hover:opacity-30 transition-opacity"></div>
                            <h3 class="text-xs font-bold text-red-600 dark:text-red-400 uppercase mb-2 flex items-center gap-1">
                    ...
                    <!-- AI Daily Summary -->
                    <div class="lg:col-span-2 p-6 bg-gradient-to-br from-purple-50 to-white dark:from-purple-900/10 dark:to-white/5 border border-purple-100 dark:border-purple-500/20 rounded-2xl shadow-sm relative">
                        <span class="material-symbols-outlined absolute top-4 right-4 text-purple-300 text-6xl opacity-20 animate-pulse">auto_awesome</span>
                        ...
                        <ul class="space-y-2">
                            <li v-for="(insight, i) in aiInsights" :key="i" class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                <!-- Entity Stats Overview Table -->
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden animate-fade-in-up">
            ...
            <div v-if="activeTab === 'financials'" class="space-y-6 animate-fade-in">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <!-- Recon Breakdown -->
                    <div class="lg:col-span-2 p-6 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm">
                        <h3 class="font-bold text-gray-900 dark:text-white mb-4">Payment Reconciliation (COD vs Digital)</h3>
                ...
                <div class="p-6 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm">
                   ...
                   <div class="mt-4 p-3 bg-orange-50 dark:bg-orange-500/10 border border-orange-100 dark:border-orange-500/20 rounded-lg flex items-center gap-3">
                         <span class="material-symbols-outlined text-orange-600">visibility</span>
                         <p class="text-xs text-orange-800 dark:text-orange-300"><strong>Anomaly Detected:</strong> Vehicle V-402 reported 450km but GPS logs show 320km on Feb 22.</p>
                   </div>
                </div>

                <!-- Payment Ledger -->
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex justify-between">
                     ...
                     <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <tr v-for="tx in store.filteredTransactions" :key="tx.id" class="hover:bg-gray-50 dark:hover:bg-white/5">
                                <td class="p-4 font-bold text-gray-900 dark:text-white">{{ tx.desc }}</td>
                                <td class="p-4 font-mono text-gray-500">{{ tx.id }}</td>
                                <td class="p-4 text-right font-mono font-bold" :class="tx.amount > 0 ? 'text-green-600' : 'text-red-500'">${{ Math.abs(tx.amount) }}</td>
                 ...
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden mt-6">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-red-50 dark:bg-red-900/10 flex justify-between">
                     ...
                     <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="p-4"><span class="bg-red-100 text-red-700 px-2 py-1 rounded text-[10px] font-bold">CRITICAL</span></td>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="p-4"><span class="bg-orange-100 text-orange-700 px-2 py-1 rounded text-[10px] font-bold">Overdue</span></td>
                 ...
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden mt-6">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-blue-50 dark:bg-blue-900/10 flex justify-between">
                     ...
                     <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="p-4"><span class="bg-green-100 text-green-700 px-2 py-1 rounded text-[10px] font-bold">PAID</span></td>
                            </tr>
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="p-4"><span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-[10px] font-bold">PROCESSING</span></td>
                            </tr>
                        </tbody>
                    </table>
                 </div>
            ...
            <div v-if="activeTab === 'attendance'" class="space-y-6 animate-fade-in">
                <!-- Overview Stats Cards -->
                 <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                     <div class="p-5 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm text-center">
                 ...
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                     ...
                     <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <tr v-for="user in store.filteredUsers" :key="user.email" class="hover:bg-gray-50 dark:hover:bg-white/5">
                 ...
                 <!-- Automated Payroll History -->
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden mt-6">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-green-50 dark:bg-green-900/10 flex justify-between">
            ...
            <div v-if="activeTab === 'workforce'" class="space-y-6 animate-fade-in">
                 <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-1 p-6 bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm">
                 ...
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm p-6 mb-6">
                    <h3 class="font-bold text-gray-900 dark:text-white mb-4">Warehouse Dwell Time Analytics (Loading/Unloading)</h3>
                    <div class="h-74 relative">
                         ...
                         <table class="w-full text-left text-xs">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                                <tr v-for="user in store.filteredUsers" :key="user.email" class="hover:bg-gray-50 dark:hover:bg-white/5">
                                    ...
                                    <td class="p-3">
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100">{{ user.role }}</span>
                                    </td>
                 ...
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                     ...
                     <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <tr v-for="d in store.filteredDrivers" :key="d.id" class="hover:bg-gray-50 dark:hover:bg-white/5">
                                ...
                                <td class="p-3 text-center">
                                     <div class="flex items-center justify-center gap-1 text-yellow-500">
                                         <span class="font-bold text-gray-900 dark:text-white">{{ d.rating }} - 4.8</span> <!-- Mock rating if missing -->
                                         <span class="material-symbols-outlined text-[14px]">star</span>
                                     </div>
                                </td>
                                <td class="p-3 text-center font-bold" :class="d.status === 'breakdown' ? 'text-red-500' : 'text-gray-300'">{{ d.status === 'breakdown' ? 1 : 0 }}</td>
            ...
            <!-- 5. ASSETS & INVENTORY (Specific Items) -->
             <div v-if="activeTab === 'assets'" class="space-y-6 animate-fade-in">
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     ...
                     <div class="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-500/20">
                         <h4 class="text-xs uppercase font-bold text-blue-800 dark:text-blue-300">Total Boxes</h4>
                         <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ getInventoryCount(['Box', 'Packaging']) }}</p>
                     </div>
                     <div class="p-4 bg-orange-50 dark:bg-orange-900/10 rounded-xl border border-orange-100 dark:border-orange-500/20">
                        <h4 class="text-xs uppercase font-bold text-orange-800 dark:text-orange-300">Crates & Cargo</h4>
                        <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ getInventoryCount(['Crates', 'Cargo']) }}</p>
                    </div>
                    <div class="p-4 bg-purple-50 dark:bg-purple-900/10 rounded-xl border border-purple-100 dark:border-purple-500/20">
                        <h4 class="text-xs uppercase font-bold text-purple-800 dark:text-purple-300">Tape & Tools</h4>
                        <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ getInventoryCount(['Tape', 'Consumables']) }}</p>
                    </div>
                    <div class="p-4 bg-teal-50 dark:bg-teal-900/10 rounded-xl border border-teal-100 dark:border-teal-500/20">
                        <h4 class="text-xs uppercase font-bold text-teal-800 dark:text-teal-300">Utensils</h4>
                        <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">{{ getInventoryCount(['Utensils']) }}</p>
                    </div>
                 </div>

                 <!-- Returnable Equipment Audit -->
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden mt-6 mb-6">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-indigo-50 dark:bg-indigo-900/10">
                        <h3 class="font-bold text-indigo-900 dark:text-indigo-300">Returnable Equipment Audit (Crates/Blankets)</h3>
                    </div>
                     <table class="w-full text-left text-xs">
                        <thead class="bg-gray-50 dark:bg-black/20 text-gray-500 dark:text-gray-400 font-bold uppercase">
                            <tr>
             ...
             <div v-if="activeTab === 'returns'" class="space-y-6 animate-fade-in">
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex gap-4 items-center">
                        <h3 class="font-bold text-gray-900 dark:text-white">RMA Returns Processing</h3>
                        <span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs font-bold">{{ store.filteredReturns.length }} Active</span>
                    </div>
                     <table class="w-full text-left text-xs">
                        <thead class="bg-gray-50 dark:bg-black/20 text-gray-500 dark:text-gray-400 font-bold uppercase">
                            <tr>
                         ...
                         <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                             <tr v-for="rma in store.filteredReturns" :key="rma.id" class="hover:bg-gray-50 dark:hover:bg-white/5">
                                 <td class="p-4 font-mono font-bold">{{ rma.id }}</td>
                                 <td class="p-4">{{ rma.customer }}</td>
                                 <td class="p-4 font-mono text-blue-600 font-bold cursor-pointer hover:underline">#ORD-9921</td>
                                 <td class="p-4">{{ rma.reason }}</td>
                                 <td class="p-4 whitespace-nowrap">
                                     <span class="px-2 py-1 rounded border" :class="rma.condition === 'Damaged' ? 'bg-red-50 border-red-200 text-red-600' : 'bg-green-50 border-green-200 text-green-600'">{{ rma.condition }}</span>
                                 </td>
                                 <td class="p-4 text-center">
                                     <button v-if="rma.images.length" class="text-blue-500 hover:underline hover:text-blue-600">View {{ rma.images.length }}</button>
                                     <span v-else class="text-gray-400">-</span>
                                 </td>
                                 <td class="p-4">
                                     <span class="font-bold" :class="{'Pending': 'text-yellow-500', 'Approved': 'text-green-500'}[rma.status]">{{ rma.status }}</span>
                                 </td>
                             </tr>
                         </tbody>
                    </table>
                 ...
                 <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                     <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm p-6">
            ...
            <div v-if="activeTab === 'security_audit'" class="space-y-6 animate-fade-in">
                 <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
                    <div class="p-4 border-b border-gray-100 dark:border-white/10 bg-gray-900 text-white flex justify-between items-center">
                        <h3 class="font-bold flex items-center gap-2">
                             <span class="material-symbols-outlined text-green-400">gpp_good</span> Immutable System Audit Ledger
                        </h3>
                        <span class="text-xs text-gray-400 font-mono">Last Sync: Live</span>
                    </div>
                     <table class="w-full text-left text-xs">
                        <thead class="bg-gray-100 dark:bg-black/40 text-gray-500 dark:text-gray-400 font-bold uppercase">
                            <tr>
                         ...
                         <tbody class="divide-y divide-gray-100 dark:divide-white/5 font-mono">
                             <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                 ...
                                 <td class="p-4 font-bold text-blue-600">admin@logistics.com</td>
                                 <td class="p-4"><span class="text-yellow-600 bg-yellow-100 px-2 py-0.5 rounded">UPDATE_PAY_RATE</span></td>
                                 <td class="p-4">Driver: Mike Ross</td>
                                 <td class="p-4 text-gray-600">Changed base rate from $22/hr to $24/hr</td>
                                 <td class="p-4 text-right text-gray-400">192.168.1.45</td>
                             </tr>
                             <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                 <td class="p-4 text-gray-400">Feb 24, 09:15:00 AM</td>
                                 <td class="p-4 font-bold text-blue-600">sys_auto</td>
                                 <td class="p-4"><span class="text-red-600 bg-red-100 px-2 py-0.5 rounded">DELETE_ORDER</span></td>
                                 <td class="p-4">Order #99212</td>
                                 <td class="p-4 text-gray-600">Auto-cancellation due to customer request</td>
                                 <td class="p-4 text-right text-gray-400">Internal</td>
                             </tr>
                             <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                                 <td class="p-4 text-gray-400">Feb 23, 11:05:44 PM</td>
                                 <td class="p-4 font-bold text-blue-600">manager_sarah</td>
                                 <td class="p-4"><span class="text-purple-600 bg-purple-100 px-2 py-0.5 rounded">MODIFY_GEOFENCE</span></td>
                                 <td class="p-4">Zone: North-Hub</td>
                                 <td class="p-4 text-gray-600">Expanded radius by 500m</td>
            ...
            <div v-if="activeTab === 'reports_history'" class="space-y-6 animate-fade-in">
                <!-- Analytics Charts -->
                 <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                     <div class="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-xl p-6 shadow-sm">
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div v-for="report in store.filteredReports" :key="report.id" 
                        ...
                        <div class="p-2.5 rounded-lg shadow-sm" :class="`bg-${report.color}-50 text-${report.color}-600 dark:bg-${report.color}-500/10 dark:text-${report.color}-400`">
                            <span class="material-symbols-outlined">{{ report.icon }}</span>
                        </div>
                        <div>
                            <div class="font-bold text-gray-900 dark:text-white text-sm group-hover:text-primary transition-colors leading-tight mb-1">
                                {{ report.title }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">Date: {{ report.date }} • {{ activeWarehouseName }}</div>
                            <button @click.stop="downloadReport(report.id)" class="mt-2 text-[10px] font-bold text-primary flex items-center gap-1 hover:underline">
                                <span class="material-symbols-outlined text-[12px]">download</span> Download PDF
                            </button>
                        </div>
                    </div>
                     <!-- Previous Report Placeholder -->
                     <button class="p-4 border-2 border-dashed border-gray-200 dark:border-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-primary hover:border-primary/50 transition-colors gap-2">
        ...
        <div v-if="notification" class="fixed bottom-6 right-6 z-50 animate-fade-in-up transition-all cursor-pointer" @click="notification = null">
            ...
            :class="notification.type === 'success' ? 'bg-green-50/90 dark:bg-green-900/90 border-green-200 dark:border-green-700 text-green-800 dark:text-green-100' : 'bg-blue-50/90 dark:bg-blue-900/90 border-blue-200 dark:border-blue-700 text-blue-800 dark:text-blue-100'">
                <span class="material-symbols-outlined text-2xl">{{ notification.type === 'success' ? 'check_circle' : 'info' }}</span>
                <div>
