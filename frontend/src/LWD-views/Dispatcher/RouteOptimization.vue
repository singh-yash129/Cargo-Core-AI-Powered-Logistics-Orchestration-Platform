        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <button @click="runOptimizer" :disabled="optimizing"
                    class="bg-primary hover:bg-primary-dark text-background-dark font-bold py-2 px-4 rounded-lg flex items-center gap-2 transition-colors disabled:opacity-50">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select v-model="optimizationGoal" class="w-full bg-gray-100 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-lg p-2 text-gray-900 dark:text-white text-sm">
                            ...
                            <option>Minimize Empty Miles</option>
                        </select>
                    </div>
                    ...
                    <div class="pt-2 border-t border-gray-200 dark:border-white/5">
                        ...
                        <div class="space-y-1 text-xs">
                            ...
                            <div class="flex items-center gap-2 text-green-400"><span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> Road restrictions</div>
                            <div class="flex items-center gap-2 text-green-400"><span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> No-go zones (from Manager)</div>
                            <div class="flex items-center gap-2 text-yellow-400"><span class="w-1.5 h-1.5 rounded-full bg-yellow-500"></span> Weather warnings (partial)</div>
                            <div class="flex items-center gap-2 text-green-400"><span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> Delivery time windows</div>
                        </div>
                    </div>
                ...
                <div v-if="showManualOverride"
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="pt-2 border-t border-gray-200 dark:border-white/5 space-y-2">
                            ...
                            <button @click="reorderStops" class="w-full text-xs py-1.5 rounded font-bold transition-colors"
                                :class="stopsReordered ? 'bg-blue-500/30 text-blue-300' : 'bg-blue-500/20 hover:bg-blue-500/30 text-blue-400'">
                                {{ stopsReordered ? '✓ Stops Reordered' : 'Reorder Stops' }}
                            </button>
                        </div>
                        <div class="text-[9px] text-gray-600 italic">All overrides logged in audit trail</div>
                    </div>
                </div>
                ...
                <div class="absolute top-4 left-4 z-10 bg-black/80 backdrop-blur border border-gray-200 dark:border-white/10 rounded-xl p-4 w-64">
                    ...
                    <div class="space-y-2 text-xs">
                        ...
                        <div class="flex justify-between">
                            <span class="text-gray-400">Delay Probability</span>
...
function adjustRoutes() {
    routeStats.distance = Math.max(600, routeStats.distance - 30)
    routeStats.efficiency = Math.min(99, routeStats.efficiency + 1)
    routesApplied.value = false
}
