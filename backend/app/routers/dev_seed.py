...
async def seed_gps_coordinates(
    ...
    return {
        ...
        **summary,
        "next_steps": [
            "Open Dispatcher → Smart Driver Assignment",
            "Click Refresh — AI suggestions will appear within a few seconds",
            "Return trip suggestions appear if any DELIVERED orders were seeded",
        ],
    }
...
async def clear_seeded_gps(
    ...
    await db.commit()
    return {"status": "cleared", "orders_cleared": len(orders), "drivers_cleared": len(profiles)}
