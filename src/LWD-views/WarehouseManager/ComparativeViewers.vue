                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div
                                ...
                                <div class="text-xl font-black text-gray-900 dark:text-white">{{
                                    getZoneMetricValue(zone.id, 'orders_processed', 100, 500) }}</div>
                            ...
                            <div
                                ...
                                <div class="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold">Picking
                                    Accuracy</div>
                                <div class="text-xl font-black text-green-500">{{ Math.min(100, getZoneMetricValue(zone.id,
                                    'picking_accuracy', 90, 100)) }}%</div>
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                ...
                                <div class="w-16 h-16 rounded-full border-4 flex items-center justify-center font-bold text-sm text-gray-900 dark:text-white"
                                    ...
                                    {{ zone.value === 'Full' ? '100%' : (zone.value || getZoneMetricValue(zone.id, 'capacity_used', 65, 95)
                                    + '%') }}
                                </div>
                                <div class="flex-1 space-y-1">
                                    ...
                                    <div class="flex justify-between text-xs">
                                        ...
                                        <span class="font-bold text-indigo-500">{{ (getZoneMetricValue(zone.id, 'capacity_used', 65,
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-lg font-bold text-gray-900 dark:text-white">{{
                                        getZoneMetricValue(zone.id, 'active_pickers', 5, 15) }}/15</div>
                                </div>
                            </div>
...
async function fetchZoneMetrics(zoneId) {
    ...
    try {
        ...
        let dateRange = 'today'
        if (timeRange.value === '7d') dateRange = 'week'
        else if (timeRange.value === '30d') dateRange = 'month'
        else if (timeRange.value === 'ytd') dateRange = 'year'

        const response = await fetch(
            `http://localhost:8000/api/v1/warehouses/${warehouseId.value}/operations/zones/${zoneId}/metrics?time_range=${dateRange}`,
            { headers }
        )

        if (response.ok) {
            const metrics = await response.json()
            zoneMetricsCache.value[zoneId] = metrics
        }
    } catch (error) {
...
// Get zone metric value - uses API data when available, falls back to pseudo-random
const getZoneMetricValue = (zoneId, metricKey, min, max) => {
    const metrics = zoneMetricsCache.value[zoneId]
    if (metrics) {
        // Map metric keys to API response fields
        const fieldMap = {
        ...
        if (fieldMap[metricKey] !== undefined) {
            return Math.round(fieldMap[metricKey])
        }
    }
    // Fallback to pseudo-random value
    return getDynamicValue(zoneId, min, max)
}
