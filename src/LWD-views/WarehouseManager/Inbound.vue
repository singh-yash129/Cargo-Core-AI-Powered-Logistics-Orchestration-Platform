...
async function fetchInboundShipments() {
    ...
    try {
        ...
        const [vendorRes, ordersRes] = await Promise.allSettled([
            fetch('http://localhost:8000/api/v1/vendors?page=1&page_size=20', {
                headers: {
                    'Authorization': `Bearer ${authStore.authToken}`,
                    'Content-Type': 'application/json'
                }
            }),
            fetch('http://localhost:8000/api/v1/orders?page=1&page_size=100&status_filter=CONFIRMED', {
                headers: {
                    'Authorization': `Bearer ${authStore.authToken}`,
                    'Content-Type': 'application/json'
                }
        ...
        if (vendorRes.status === 'fulfilled' && vendorRes.value.ok) {
            const data = await vendorRes.value.json()
            vendors = data.items || data || []
        }
        ...
        dockSchedule.value = asns.value.slice(0, 6).map((asn, idx) => {
            const scheduledDate = vendorOrders[idx]?.scheduled_at ? new Date(vendorOrders[idx].scheduled_at) : null
            const timeStr = scheduledDate
                ? scheduledDate.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: false })
                : `Dock Slot ${idx + 1}`
            const isCompleted = asn.status === 'Completed'
            const isActive = asn.status === 'Arrived' || asn.status === 'Receiving'
            const statusMap = isCompleted ? 'Completed' : isActive ? 'Active' : 'Scheduled'
            const statusClassMap = {
            ...
            return {
                ...
                dock: `Dock ${(idx % 4) + 1}`,
                pallets: Math.max(1, Math.ceil(asn.expected / 10)),
                status: statusMap,
                statusClass: statusClassMap[statusMap]
            }
        })

    } catch (error) {
        console.error('Error fetching inbound shipments:', error)
        showToast('Unable to load inbound data from server')
    } finally {
        loading.value = false
    }
}
