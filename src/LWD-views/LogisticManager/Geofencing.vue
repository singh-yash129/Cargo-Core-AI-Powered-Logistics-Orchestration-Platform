    ...
    <div class="h-[calc(100vh-8rem)] flex flex-col gap-6">
            ...
            <div class="flex gap-3">
                <div class="flex items-center gap-1 bg-gray-100 dark:bg-black/40 rounded-lg p-1 border border-gray-200 dark:border-white/10 shadow-sm">
                    <button @click="viewMode = 'map'" 
                        ...
                        :class="viewMode === 'map' ? 'bg-slate-800 dark:bg-white/10 text-white shadow-sm border border-slate-700 dark:border-white/10' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'">
                        <span class="material-symbols-outlined text-[18px]">map</span> Map
                    </button>
                    <button @click="viewMode = 'list'"
                        class="px-3 py-1.5 rounded text-sm font-medium transition-all flex items-center gap-2"
                        :class="viewMode === 'list' ? 'bg-slate-800 dark:bg-white/10 text-white shadow-sm border border-slate-700 dark:border-white/10' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'">
        ...
        <div class="flex-1 flex gap-6 overflow-hidden min-h-0">
            ...
            <div class="w-80 glass-panel rounded-xl flex flex-col border border-gray-200 dark:border-white/5 flex-shrink-0">
                ...
                <div class="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                    <div v-for="zone in searchedZones" :key="zone.id"
                        ...
                        :class="selectedZone?.id === zone.id ? 'bg-primary/5 border-primary/30' : 'border-transparent bg-slate-50 dark:bg-white/5'">
            ...
            <div class="flex-1 glass-panel rounded-xl relative overflow-hidden border border-gray-200 dark:border-white/5 flex flex-col">
                ...
                <div v-if="viewMode === 'map'" class="absolute inset-0 bg-gray-100 dark:bg-gray-900 flex flex-col">
                    <!-- Tools Overlay -->
                    <div class="absolute top-4 left-4 z-10 flex flex-col gap-2 bg-slate-50/90 dark:bg-black/80 p-2 rounded-lg backdrop-blur shadow-lg border border-gray-200 dark:border-white/10">
                    ...
                    <div class="flex-1 bg-[url('https://maps.googleapis.com/maps/api/staticmap?center=40.7128,-74.0060&zoom=12&size=800x600&sensor=false')] bg-cover bg-center grayscale opacity-80 dark:opacity-40 relative group overflow-hidden">
                        ...
                        <div v-for="zone in searchedZones" :key="zone.id" 
                            ...
                            @click="selectZone(zone)">
                            <div class="bg-slate-50/90 dark:bg-black/80 px-2 py-1 rounded text-[10px] font-bold shadow-sm backdrop-blur-sm whitespace-nowrap">
                                {{ zone.name }}
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Bottom Info Bar -->
                    <div class="h-10 bg-slate-50 dark:bg-card-dark border-t border-gray-200 dark:border-white/10 flex items-center px-4 justify-between text-xs text-gray-500">
        ...
        <Teleport to="body">
            <div v-if="isZoneModalOpen" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div class="bg-slate-50 dark:bg-gray-900 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-scale-in border border-gray-200 dark:border-white/10">
                    ...
                    <div class="p-6 pt-0 flex justify-end gap-3">
                        <button @click="closeZoneModal" class="px-4 py-2 text-sm font-medium text-white bg-slate-800 hover:bg-slate-700 dark:bg-white/10 dark:hover:bg-white/15 dark:text-gray-200 rounded-lg border border-slate-700 dark:border-white/10">Cancel</button>
