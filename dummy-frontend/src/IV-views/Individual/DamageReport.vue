    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select v-model="form.orderId"
                            ...
                            <option value="" class="bg-white dark:bg-gray-800">Choose an order...</option>
                            <option v-for="o in store.deliveredOrders" :key="o.id" :value="o.id" class="bg-white dark:bg-gray-800">
                                {{ o.id }} — {{ o.cargoType }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div @click="$refs.photoInput?.click()"
                            ...
                            <span class="text-xs mt-1">
                                {{ photos.length > 0 ? photos.length + ' file(s) selected' : 'Before / After photos recommended' }}
                            </span>
                        </div>
                        ...
                        <div v-if="photos.length > 0" class="flex gap-2 flex-wrap mt-2">
                            <div v-for="(p, i) in photos" :key="i"
                                class="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-white/5 flex items-center justify-center">
                                <img v-if="p.preview" :src="p.preview" class="w-full h-full object-cover" />
                                <span v-else class="material-symbols-outlined text-2xl text-gray-400">movie</span>
                                <button @click.stop="removePhoto(i)"
                                    class="absolute top-0.5 right-0.5 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-white">
                            ...
                            <button type="button"
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Faster resolution based on submitted photos. No pickup arranged.</div>
                                    </div>
                                </div>
                            </button>
                            ...
                            <button type="button"
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">A driver collects your parcel for physical inspection at the warehouse.</div>
                                    </div>
                                </div>
                            </button>
                        </div>
            ...
            <div class="space-y-4">
                ...
                <!-- Status timelines for both flows -->
                <div class="glass-panel p-4 sm:p-5 rounded-xl space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1 flex-wrap text-[10px]">
                            <span v-for="(s, i) in flow1Statuses" :key="s">
                                <span class="px-2 py-1 rounded-full font-bold" :class="flow1StatusClass(s)">{{ s }}</span>
                                <span v-if="i < flow1Statuses.length - 1" class="material-symbols-outlined text-gray-300 dark:text-gray-600 text-xs mx-0.5">arrow_forward</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1 flex-wrap text-[10px]">
                            <span v-for="(s, i) in flow2Statuses" :key="s">
                                <span class="px-2 py-1 rounded-full font-bold" :class="flow2StatusClass(s)">{{ s }}</span>
                                <span v-if="i < flow2Statuses.length - 1" class="material-symbols-outlined text-gray-300 dark:text-gray-600 text-xs mx-0.5">arrow_forward</span>
                            </span>
                        </div>
                    </div>
                </div>
...
const form = reactive({
    orderId: '',
    description: '',
    resolutionType: '',       // 'photo_review' | 'pickup_inspection'
...
function flow1StatusClass(s) {
    if (s === 'Reported') return 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400'
    if (s === 'Under Review') return 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400'
    if (s === 'Claims Reviewed') return 'bg-teal-100 text-teal-700 dark:bg-teal-500/20 dark:text-teal-400'
    if (s === 'Approved') return 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400'
    if (s === 'Refunded') return 'bg-green-200 text-green-800 dark:bg-green-500/30 dark:text-green-300'
    return 'bg-gray-100 text-gray-600 dark:bg-gray-500/20 dark:text-gray-400'
}

function flow2StatusClass(s) {
    if (s === 'Reported') return 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400'
    if (s === 'Pickup Requested') return 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400'
    if (['Pickup Approved', 'Pickup Scheduled'].includes(s)) return 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400'
    if (s === 'At Warehouse') return 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400'
    if (s === 'Physically Inspected') return 'bg-teal-100 text-teal-700 dark:bg-teal-500/20 dark:text-teal-400'
    if (s === 'Approved') return 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400'
    if (s === 'Refunded') return 'bg-green-200 text-green-800 dark:bg-green-500/30 dark:text-green-300'
    return 'bg-gray-100 text-gray-600 dark:bg-gray-500/20 dark:text-gray-400'
}
...
function getStatusClass(status) {
    const map = {
        'Reported': 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400',
        'Under Review': 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400',
        'Claims Reviewed': 'bg-teal-100 text-teal-700 dark:bg-teal-500/20 dark:text-teal-400',
        'Pickup Requested': 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400',
        'Pickup Approved': 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400',
        'Pickup Scheduled': 'bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-300',
        'Collected': 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400',
        'At Warehouse': 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400',
        'Physically Inspected': 'bg-teal-100 text-teal-700 dark:bg-teal-500/20 dark:text-teal-400',
        'Approved': 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400',
        'Partially Approved': 'bg-lime-100 text-lime-700 dark:bg-lime-500/20 dark:text-lime-400',
        'Rejected': 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400',
        'Refunded': 'bg-green-200 text-green-800 dark:bg-green-500/30 dark:text-green-300',
