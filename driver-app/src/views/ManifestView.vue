<template>
    <div class="screen-layout" :class="isDark ? 'bg-background-dark text-white' : 'bg-background-light text-gray-900'">
        ...
        <!-- ── SCROLLABLE BODY ───────────────────────── -->
        <div class="screen-body px-5 py-4 relative">
            <!-- Timeline line -->
            <div class="absolute left-[3.35rem] top-0 bottom-0 w-0.5 timeline-line opacity-20 pointer-events-none">
            </div>

            <!-- Current location -->
            <div class="relative z-10 mb-6 flex items-center">
                <div class="flex flex-col items-center mr-4 min-w-[3rem]">
                    <div class="w-3 h-3 rounded-full bg-primary shadow-[0_0_10px_rgba(28,231,131,0.6)] animate-pulse">
                    </div>
                </div>
            ...
            <!-- Stop cards -->
            <div v-for="(stop, idx) in stops" :key="stop.id" class="relative z-10 mb-4 flex items-start">
                <!-- Stop number dot -->
                <div class="flex flex-col items-center mr-4 min-w-[3rem] pt-1">
                    <div class="flex items-center justify-center font-bold z-10 w-8 h-8 rounded-full border text-sm"
                        :class="[
                ...
                <div @click="$router.push(`/stop/${stop.id}`)"
                    class="flex-1 rounded-2xl p-4 cursor-pointer transition-all active:scale-[0.98] border"
                    ...
                    <div class="flex items-center justify-between pt-3 border-t"
                        ...
                        <div class="flex items-center gap-3 text-xs"
                            ...
                            <span class="flex items-center gap-1"><span class="material-icons text-xs">schedule</span>{{
                        ...
                        <button v-if="idx === 0" @click.stop="$router.push('/navigation')"
                            class="bg-primary text-background-dark text-xs font-black py-1.5 px-4 rounded-lg flex items-center gap-1 active:scale-95">
