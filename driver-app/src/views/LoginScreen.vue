...
const handleLogin = async () => {
    ...
    try {
        ...
        await driverStore.login(driverId.value.trim(), password.value)
        // Navigate to pre-shift on successful login
        router.push('/pre-shift')
    } catch (err) {
        error.value = err.message || 'Invalid Driver ID or Password'
    } finally {
        loading.value = false
    }
}
