...
async def google_login_or_register(db: AsyncSession, redis: Redis, data: GoogleLoginRequest) -> TokenResponse:
    try:
        # User needs to put their GOOGLE_CLIENT_ID in env, or we skip client_id check if not strictly required in dev 
        # but google validation requires setting audience. For demo, we verify without audience check if we don't have it.
        # It's better to verify properly:
